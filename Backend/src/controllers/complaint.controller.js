...
const deleteComplaint = asyncHandler(async (req, res) => {
  ...
  if (!complaint) {
    return res
      .status(404)
      .json(new ApiResponse(404, null, "Complaint not found"));
  }
  ...
  if (complaint.student.toString() !== req.user._id.toString()) {
    return res
      .status(403)
      .json(
        new ApiResponse(403, null, "You can delete only your own complaints"),
      );
  }
...
const getAllComplaints = asyncHandler(async (req, res) => {
  const complaints = await Complaint.find()
    .populate("student", "fullName rollNo hostelName roomNo")
    .sort({
      createdAt: -1,
    });
