...
const registerStudent = asyncHandler(async (req, res) => {
  const { fullName, rollNo, year, hostelName, roomNo, collegeEmail, password } =
    req.body;

  // agar missing hai koi bhi field to error de dena
  if (
    [fullName, rollNo, year, hostelName, roomNo, collegeEmail, password].some(
      (field) => !field,
    )
  ) {
    return res
  ...
  if (existedStudent) {
    return res
      .status(409)
      .json(new ApiResponse(409, null, "Student already exists"));
  }
...
const getCurrentStudent = asyncHandler(async (req, res) => {
  return res
    .status(200)
    .json(
      new ApiResponse(200, req.user, "Current student fetched successfully"),
    );
});
export {
  registerStudent,
